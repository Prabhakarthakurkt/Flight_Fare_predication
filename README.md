...
Feature Engineering
    ...
    total_df['Date']=total_df['Month'].astype(int)
    total_df['Date']=total_df['Year'].astype(int)
...
Let's see if Additional info has any effect on price
    ...
    sns.catplot(y='price',x='Additional_Info',data=df.sort_values('price',ascending=False),kind="boxen",hight=6,aspect=3)
    plt.show
Filering the Arrival Time 
...
Let's us make the stop colume numeric
...
Similarly we will try to convert all the feature to numerical variables
...
we can also perform One Hot Enoding on the categorical variables
...
Hypertuning our Model
    ...
    param_grid = {'n_estimators': n_estimators,
     ...
     param_grid,scoring='neg_mean_squared_error', n_iter = 50, cv = 5,
...
auto-sklearn frees a machine learning user from algorithms selection and hyperparameter tunig.
it leverage recent advantages in Bayesian optimization, meta-learning and ensemble construction
